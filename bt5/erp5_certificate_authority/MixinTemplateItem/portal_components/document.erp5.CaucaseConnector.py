...
class CaucaseConnector(XMLObject):
  ...
  def _getAuthenticatedServiceConnection(self):
    ...
    with tempfile.NamedTemporaryFile(prefix='caucase_user_', **{B_ARG: 0}) as user_key_file:
      user_key_file.write(str2bytes(self.getUserKey()))
      user_key_file.write(str2bytes("\n"))
      user_key_file.write(str2bytes(self.getUserCertificate()))
      user_key_file.flush()
  ...
  def _getSubjectNameAttributeList(self):
    ...
    if crt_pem is None:
      ...
      for oid, value in [
        ...
        if value:
          name_attribute_list.append(x509.NameAttribute(oid, str2unicode(value)))
      return name_attribute_list
    else:
      # Extract name attributes from the existing crt
      ssl_certificate = x509.load_pem_x509_certificate(str2bytes(crt_pem))
  ...
  def updateCACertificateChain(self):
    with tempfile.NamedTemporaryFile(prefix='caucase_ca_certificate_chain_', **{B_ARG: 0}) as ca_crt_file:
      if self.getCaCertificateChain():
        ca_crt_file.write(str2bytes(self.getCaCertificateChain()))
        ca_crt_file.write(str2bytes("\n"))
        ca_crt_file.flush()
        ca_crt_file.seek(0)
  ...
  def verifyCertificate(self, crt_pem):
    ...
    load_certificate(
      crt_pem, [load_ca_certificate(str2bytes(self.getCaCertificateChain()))], [])
    return crt_pem
